  ...
  return (
    <div className={cn(
      ...
      <div className={cn(
        ...
        <div className={cn(
          "grid gap-4",
          isFullscreen ? "grid-cols-1 flex-1" : "grid-cols-1 lg:grid-cols-4"
        )}>
          {/* Chart Container */}
          <Card className={cn(
            "border-border bg-card overflow-hidden",
            isFullscreen ? "flex-1 rounded-none border-0" : "lg:col-span-3"
          )}>
          ...
          <div className="text-center text-xs text-muted-foreground mt-4">
            ...
            {data && (
              <p className="mt-1 opacity-70">
                مصدر البيانات: {data.dataSource} • آخر تحديث: {new Date(data.timestamp).toLocaleTimeString('ar-SA')}
              </p>
            )}
          </div>
        )}
