...
export const TradeHistory = () => {
  ...
  return <Card>
    <CardHeader>
      ...
      {/* Stats Summary */}
      {stats.total > 0 && <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-4">
    ...
    <CardContent>
      <ScrollArea className="h-[400px]">
        {myTrades.length === 0 ? <div className="flex flex-col items-center justify-center h-[300px] text-center">
            <p className="text-muted-foreground">لم تسجل أي نتائج بعد</p>
            <p className="text-xs text-muted-foreground mt-2">
              سجل نتائج صفقاتك من قسم التوصيات المباشرة
            </p>
          </div> : <div className="space-y-3">
            {myTrades.map(signal => signal && <div key={signal.id} className="flex items-center justify-between gap-3 rounded-lg border border-border bg-card p-4 hover:bg-accent/50 transition-colors">
                <div className="flex items-center gap-3 flex-1">
                  <div className={cn("flex h-10 w-10 items-center justify-center rounded-lg shrink-0", signal.direction === "CALL" ? "bg-success/20" : "bg-danger/20")}>
                    {signal.direction === "CALL" ? <TrendingUp className="h-5 w-5 text-success" /> : <TrendingDown className="h-5 w-5 text-danger" />}
                  </div>
                ...
                <Badge variant={signal.userResult === 'win' ? 'default' : 'destructive'} className={cn("gap-1 text-base px-3 py-1", signal.userResult === 'win' && 'bg-success hover:bg-success/90')}>
